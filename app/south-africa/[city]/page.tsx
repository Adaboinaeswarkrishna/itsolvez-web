...
export function generateStaticParams() {
  return getIntlCitiesByCountry("south-africa").map((c) => ({ city: c.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  const { city } = await params;
  const data = getIntlCityBySlug("south-africa", city);
  if (!data) return {};
  return buildMetadata({
    title: data.metaTitle, description: data.metaDescription, keywords: data.keywords,
    slug: `south-africa/${data.slug}`, ogImage: "/og-image.png",
  });
}
