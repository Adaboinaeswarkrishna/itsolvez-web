...
export function generateStaticParams() {
  return getIntlCitiesByCountry("saudi-arabia").map((c) => ({ city: c.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  const { city } = await params;
  const data = getIntlCityBySlug("saudi-arabia", city);
  if (!data) return {};
  return buildMetadata({
    title: data.metaTitle, description: data.metaDescription, keywords: data.keywords,
    slug: `saudi-arabia/${data.slug}`, ogImage: "/og-image.png",
  });
}
