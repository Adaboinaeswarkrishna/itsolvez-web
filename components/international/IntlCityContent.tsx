...
import { ArrowRight, CheckCircle2, ChevronDown, Clock, MapPin } from "lucide-react";
...
import { INTL_CORE_SERVICES, getIntlCitiesByCountry, type IntlCityPage } from "@/lib/data/intl-cities";
import { pickCityHeroImage } from "@/lib/heroImagePool";

export default function IntlCityContent({ data }: { data: IntlCityPage }) {
  const siblings = getIntlCitiesByCountry(data.countrySlug).filter((c) => c.slug !== data.slug);
  const countryHref = `/${data.countrySlug}`;

  return (
    <>
      <JsonLd data={breadcrumbSchema([
        { name: "Home", url: "https://itsolvez.com/" },
        { name: data.country, url: `https://itsolvez.com${countryHref}/` },
        { name: data.city, url: `https://itsolvez.com${countryHref}/${data.slug}/` },
      ])} />
      <JsonLd data={faqSchema(data.faqs.map((f) => ({ question: f.q, answer: f.a })))} />

      <PageHero
        tag={`${data.city} · ${data.country}`}
        title={data.h1}
        titleAccent={data.h1Accent}
        subtitle={data.intro}
        bgImage={pickCityHeroImage(`${data.countrySlug}-${data.slug}`)}
        breadcrumbs={[{ label: "Home", href: "/" }, { label: data.country, href: countryHref }, { label: data.city }]}
        waveFill="#F4F7FC"
      />
      ...
      <section className="section-py bg-[#F4F7FC]">
        <div className="container-custom">
          <div className="text-center mb-12">
            <span className="section-tag mb-4">What we do for {data.city}</span>
            <h2 className="section-heading">Full-stack delivery, one accountable team</h2>
          </div>
      ...
      {/* Why outsource to India + time zone */}
      <section className="section-py bg-white">
        <div className="container-custom max-w-3xl">
          <span className="section-tag mb-5">Why {data.city} businesses choose an India-based team</span>
          <h2 className="section-heading mb-5">Built for how {data.city} does business</h2>
          <p className="text-[#5A6380] leading-relaxed mb-6">{data.whyOutsource}</p>

          <div className="bg-[#F4F7FC] rounded-2xl p-6 flex items-start gap-4 mb-8">
        ...
        <div className="container-custom max-w-3xl">
          ...
          {siblings.length > 0 && (
            <p className="mt-8 text-sm text-[#5A6380] flex items-start gap-2">
              ...
              Also serving {data.country}: {siblings.map((c, i, arr) => (
                <span key={c.slug}>
                  <Link href={`${countryHref}/${c.slug}`} className="text-[#1878F0] hover:underline" prefetch={false}>{c.city}</Link>
                  {i < arr.length - 1 ? ", " : ""}
                </span>
              ))}
            </p>
      ...
      <InquirySection source={`Intl City: ${data.city}, ${data.country}`} />
    </>
  );
}
